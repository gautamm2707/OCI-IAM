...
def load_config(file_name):
    if file_name is None:
        file_name = '/home/opc/DeviceCode/example_config.yml'
    with open(file_name, 'r') as stream:
        return yaml.safe_load(stream)


def make_authorization_request(config):
    device_response = requests.post(config['oauth']['device_endpoint'],
                                    data={'client_id': config['oauth'
                                    ]['client']['id'],
                                    'scope': ' '.join(config['oauth'
                                    ]['scope'])})

    if 'error' in device_response.json():
        raise Oauth2Exception(device_response.json()['error'],
                              device_response.json()['error_description'
                              ])
...
def print_authentication_promt(pamh, config, authorization):
    ...
    prompt(pamh, config['texts']['prompt'].format(url=url_complete,
           url_short=url, code=user_code, qr=qr_str))
...
def poll_for_token(pamh, config, authorization):
    ...
    while True:
        ...
        token_response = requests.post(config['oauth']['token_endpoint'
                ], auth=(config['oauth']['client']['id'], config['oauth'
                ]['client']['secret']),
        ...
        if 'error' in token_response.json():
            ...
            if token_response.json()['error'] \
                ...
                raise Oauth2Exception(token_response.json()['error'],
...
def make_userinfo_request(config, token_response):
    userinfo_response = requests.get(config['oauth']['userinfo_endpoint'
            ], headers={'Authorization': 'Bearer %s'
            % str(token_response['access_token'])})

    if 'error' in userinfo_response.json():
        raise Oauth2Exception(userinfo_response.json()['error'],
                              userinfo_response.json()['error_description'
                              ])
...
def authorize_user(pamh, config, userinfo):
    ...
    if sub not in config['users']:
        raise Oauth2Exception('User not found',
                              'username is not found in configuration',
                              sub)

    if pamh.user not in config['users'][sub]:
        raise Oauth2Exception('Authorization failed',
                              'Do not have sufficient permission', sub,
                              pamh.user)


def generate_qr(str, config):
    qr = \
        qrcode.QRCode(error_correction=qrcode.constants.ERROR_CORRECT_L)
    qr.add_data(str)
    qr.make()
...
def generate_qr_small(modules, config):
    ...
    for y in range(0, len(modules) // 2 + 1):
        ...
        for x in range(0, len(modules[0])):
            qr_str += qr_half_char(modules[y * 2][x], (modules[y * 2
                                   + 1][x] if len(modules) > y * 2
                                   + 1 else False), config)
        qr_str += qr_half_char(False, False, config)
        if y != len(modules) // 2:
            qr_str += after_line + '\n'
