...
class IAM():
    ...
    def getappid(self):     #2.
        ...
        for x in tempjsn:
            id = x.get("id")
        print("Deleting the EBS Asserter application")
        obj.deactivateapp(id, accesstoken)
        obj.deleteapp(id, accesstoken)


    def deactivateapp(self, id, accesstoken):
        obj = IAM()
        ...
        print("Deactivated ! Now performing the deletion")

    def deleteapp(self, id, accesstoken):
        obj = IAM()
        #accesstoken = obj.printaccesstoken()
        extra = "/admin/v1/Apps"+"/"+id
        headers = {'Accept': '*/*', 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + accesstoken}
        resp = requests.request("DELETE", idcsURL+extra, headers=headers, verify=False)
        print("Deleted the application. Now Deactivate the Domain")
