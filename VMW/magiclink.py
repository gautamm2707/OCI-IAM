...
def getUserID(user):
    if options.verbose:
        print("Obtaining UserID for %s" % user)
    header = {}
    ...
    if options.verbose:
        print("User info %s" % userInfo['Resources'][0]['id'])
...
def tokenDel():
    ...
    if r.status_code == 204:
        if options.verbose:
            print("Token successfully deleted for %s." % options.user)
        return True
    else:
        if options.verbose:
            print("Token delete FAILED for %s." % options.user)
        return False
...
def tokenReq():
    ...
    if options.verbose:
        print("Requesting Token for %s." % options.user)
...
def getKey(secret):
    ...
    #    print("REQUESTS: %s %s %s %s" % (u, header, data, r))
...
def main():
    ...
    parser.add_option("-a", "--apiuser", action="store", type="string", dest="apiuser", default="gm_cli",
                      help="Use API user and key. Best for use in scripts.")
    parser.add_option("-k", "--apikey", action="store", type="string", dest="apikey",
                      default="5FwnIOUlTRle0KjwuIizJvqwJPuJHfUP96IbpQWb19w1oW8k",
    ...
    dataStr = "%s:%s" % (options.apiuser, options.apikey)
    dataBytes = dataStr.encode('utf-8')
    secret = base64.b64encode(dataBytes).decode('utf-8')

    if options.user == "" or (options.tdel == False and options.treq == False and options.tful == False):
        print("Must include a user to operate with and select and action to perform.")
        parser.print_help()
        exit()
    ...
    if options.apikey != "" and options.apiuser != "":
        ...
        tinput = input("Press enter to use default or enter new tenant (%s):" % options.tenant)
